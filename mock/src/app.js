...
function viewReporting() {
  ...
  const mix = ['Pickup', 'SUV', 'Sedan', 'Wagon'].map((k, i) => ({
    k, n: inv.filter(v => v.body === k).length, c: ['#3D8BFF', '#0e8fa8', '#c9760a', '#15a34a'][i] }));
...
function storeHeader() {
  ...
  <div class="stbar"><div class="in">
    ...
    <a href="#/lotwalk" style="color:var(--brand-lt);font-weight:700">← back to Rooftop admin</a>
  </div></div></header>`;
}
...
function viewAdDesk() {
  ...
  /*
   * These previews crop 1:1, and a square crop of a landscape photo throws away
   * the ends of the car. On a front 3/4 that is fine — the car sits in the
   * middle of the frame. On a rear 3/4 shot of a long-bed truck you get a bed
   * and one wheel, which is what the 2021 Ram was doing here. So the ad units
   * only ever draw from units whose lead photo is a front 3/4; 22 of the 28
   * front-line units qualify, and the rest still appear everywhere else in the
   * demo where the tile is landscape.
   */
    ...
    <div class="card">
      ...
      <div class="pad">
        <div class="swatches">
          ...
          <div class="swatch"><i style="background:#3D8BFF"></i>
            <div><b>Accent</b><span>#3D8BFF</span></div></div>
